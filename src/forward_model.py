...
def save_error_map(predicted, ground_truth, output_path, title="Prediction Error", 
    ...
    """
    # Save current backend and switch to non-interactive
    import matplotlib
    ...
    try:
        ...
        # Set up the figure (don't display)
...
def loss_function_alpha(predicted, layer_outputs, target, incident, alpha_torch, use_laplacian_loss=False, slope_param=0.001):
    # Crop all inputs to the size of the target
    ...
    # Add variance of Laplacian consistency only if enabled via JSON config
    if use_laplacian_loss:
        # slope_param controls how much variance should increase per layer (from JSON config)
        var_lap_consistency_loss = variance_of_laplacian_consistency_2(alpha_torch, slope_param=slope_param) #* 0.1
    else:
        var_lap_consistency_loss = torch.tensor(0.0, device=alpha_torch.device)
    
    # Add Fourier resolution depth consistency loss
    fourier_depth_consistency_loss = fourier_resolution_depth_consistency(alpha_torch) * 0# * 10.0
    
    # Return combined loss with larger weights for new components
    total_loss = mse_loss + layer_pixel_consistency_loss + var_lap_consistency_loss + fourier_depth_consistency_loss
    print(f'total_loss: {total_loss.item()}, mse_loss: {mse_loss.item()}, layer_pixel_consistency_loss: {layer_pixel_consistency_loss.item()}, var_lap_consistency_loss: {var_lap_consistency_loss.item()}, fourier_depth_consistency_loss: {fourier_depth_consistency_loss.item()}')
...
def variance_of_laplacian(img):
    ...
    laplacian_kernel = torch.tensor([
        ...
        [0, -1, -3, -3, -3, -1, 0],
        [0, 0, -1, -1, -1, 0, 0]
    ], dtype=torch.float32).reshape(1, 1, 7, 7)
    ...
    # Apply the Laplacian filter with padding=3 for 7x7 kernel
    laplacian = F.conv2d(img, laplacian_kernel, padding=3)
...
def variance_of_laplacian_consistency_2(alpha_torch, slope_param=0.002):
    """
    Encourages the variance of the Laplacian to increase with depth.
    Superficial layers (index 0) serve as intercept, deeper layers should have 
    progressively higher variance according to slope_param.
    alpha_torch shape: [648, 648, 16, 3] for RGB
    slope_param: Controls how much variance should increase per layer (default: 0.1)
    """
    # For RGB, we need to handle each channel separately
    num_channels = alpha_torch.shape[-1]
    num_layers = alpha_torch.shape[2]
    var_lap_losses = []
    ...
    for c in range(num_channels):
        ...
        print(f'var of laps for layers 0-15 (deep->superficial), channel {c}: {var_laps}')
        
        # Create target variance pattern: layer 0 (deepest) has highest variance, layer 15 (most superficial) has lowest
        # Reverse the depth indices so that higher layer numbers (more superficial) have lower target variance
        depth_indices = torch.arange(num_layers, dtype=torch.float32, device=var_laps.device)
        reversed_indices = (num_layers - 1) - depth_indices  # Layer 15 -> 0, Layer 14 -> 1, ..., Layer 0 -> 15
        
        # Use the most superficial layer's variance as the minimum baseline
        baseline_var = var_laps[-1].detach()  # Layer 15 (most superficial) as baseline
        target_var_laps = baseline_var + slope_param * reversed_indices


        
        # Use L1 loss between actual and target variance pattern
        var_lap_loss = torch.mean(torch.abs(var_laps - target_var_laps))
        var_lap_losses.append(var_lap_loss)
    
    # Average the loss across all channels
    return torch.mean(torch.stack(var_lap_losses))

def compute_fourier_resolution(img):
    """
    Compute a simplified Fourier resolution metric for a single layer image.
    Returns a value that increases with higher spatial frequency content.
    """
    # Add batch and channel dimensions if needed
    if len(img.shape) == 2:
        img = img.unsqueeze(0).unsqueeze(0)
    ...
    # Ensure we have a 2D spatial image [B, C, H, W]
    if len(img.shape) != 4:
        raise ValueError(f"Expected 4D tensor, got shape {img.shape}")
    ...
    y, x = torch.meshgrid(
        torch.arange(H, dtype=torch.float32, device=img.device) - center_h,
        torch.arange(W, dtype=torch.float32, device=img.device) - center_w,
        indexing='ij'
    )
    ...
    # Weight power spectrum by normalized radius to emphasize high frequencies
    # Higher values indicate more high-frequency content (higher resolution)
    weighted_power = power_spectrum * radius_norm.unsqueeze(0).unsqueeze(0)
    
    # Compute mean weighted power as resolution metric
    resolution_metric = torch.mean(weighted_power)
    
    return resolution_metric

def fourier_resolution_depth_consistency(alpha_torch):
    """
    Simplified Fourier resolution depth consistency using L1 loss.
    Encourages deeper layers to have lower Fourier resolution than superficial layers.
    
    alpha_torch shape: [H, W, num_layers, 3] for RGB
    """
    num_channels = alpha_torch.shape[-1]
    num_layers = alpha_torch.shape[2]
    ...
    for c in range(num_channels):
        # Calculate Fourier resolution for each layer in this channel
        layer_resolutions = []
        for i in range(num_layers):
            layer = alpha_torch[:, :, i, c]
            resolution = compute_fourier_resolution(layer)
            layer_resolutions.append(resolution)
        
        layer_resolutions = torch.stack(layer_resolutions)  # Shape: [num_layers]
        
        # Simple approach: enforce that resolution decreases with depth
        # Create a target that decreases linearly from superficial to deep
        # Normalize to [0, 1] range where 1 is superficial, 0 is deepest
        depth_weights = torch.linspace(1.0, 0.0, num_layers, device=layer_resolutions.device)
        
        # Normalize resolutions to [0, 1] for comparison
        res_min = layer_resolutions.min()
        res_max = layer_resolutions.max()
        if res_max > res_min:
            normalized_resolutions = (layer_resolutions - res_min) / (res_max - res_min)
        else:
            normalized_resolutions = layer_resolutions
        
        # Use L1 loss to encourage resolutions to follow depth pattern
        depth_loss = torch.mean(torch.abs(normalized_resolutions - depth_weights))
        depth_consistency_losses.append(depth_loss)
    
    # Average the loss across all channels
    return torch.mean(torch.stack(depth_consistency_losses))
...
if __name__ == "__main__":
    ...
    use_laplacian_loss = config['training'].get('use_laplacian_loss', False)  # Default to False
    slope_param = config['training'].get('slope_param', 0.001)  # Default to 0.001
    ...
    if use_gui:
        try:
            ...
            print("Falling back to automatic coordinate generation.")
            source_coords = gen_source_coords(num_images, image_size, img_x_um, z_depth_mm, right_to_left, light_step, center_image, y_position)
    else:
    ...
    for epoch in range(num_epochs):
        for pos in range(num_images):
            ...
            loss = loss_function_alpha(predicted, layer_outputs, image_stack[pos], incident[pos], alpha_torch, use_laplacian_loss, slope_param)
            loss.backward()
            optimizer.step()
    ...
    if FOURIER_ANALYSIS_AVAILABLE:
        ...
        try:
            ...
            resolution_results = analyze_alpha_layers_z_resolution(
                ...
                show_plot=False,  # Don't show plots, just save them
