...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        current_config = {
            "training": {
                ...
                "learning_rate": learning_rate,  # This will be updated when we implement full JSON
                "scatter": scatter
            },
            "image_processing": {
                "image_size": image_size,
    ...
    for pos in range(num_images):
        with torch.no_grad():
            ...
            # Save ground truth (original input images, cropped)
            ground_truth_np = image_stack[pos].cpu().numpy()  # Shape: [3, H, W]
            ground_truth_np = crop_incident_to_final(ground_truth_np, input_pad)
